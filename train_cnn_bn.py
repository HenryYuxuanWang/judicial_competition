...
hidden_size = FLAGS.hidden_size  # hidden_layer
...
training = tf.placeholder_with_default(False, shape=(), name='training')  # for batch normalization
...
with tf.name_scope("max_pool"):
    pooled_brfore_bn = tf.nn.max_pool(
        h,
        ksize=[1, h.shape[1], 1, 1],  # 实现global_maxpool的功能
        strides=[1, 1, 1, 1],
        padding='VALID',
        name="pool")
